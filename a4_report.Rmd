...
Lobster size inside MPA sites increased significantly by an average of 15% from 2012 to 2018 (two-sample t-test, t(`r round(diff_mpa_t$parameter, 2)`) = `r round(diff_mpa_t$statistic, 2)`, *p* < 0.001)), while lobsters observed outside of MPAs showed a marginal decrease in mean size over the same time period that was non-significant (two-sample t-test, t(`r round(diff_non_mpa_t$parameter, 2)`) = `r round(diff_non_mpa_t$statistic, 2)`, *p* = `r round(diff_non_mpa_t$p.value, 2)`; Table 1). 
