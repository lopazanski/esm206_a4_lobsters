...
ggplot(data = lobster_counts, aes(x = year, y = annual_abundance)) +
  geom_line(aes(color = site2, label = site2)) +
  geom_label_repel(data = lobster_count_label,
             box.padding = 0.5,
             nudge_x = 0.5,
             min.segment.length = 5,
             xlim = c(2012, 2020),
             aes(label = site2, color = site2))+
  labs(
  ...
  theme_minimal() +
  scale_x_continuous(limits = c(2012, 2018), expand = c(0, 0.5)) +
  theme(
...
```
Both the MPA sites, Naples and Isla Vista, show visible increases in lobster size distribution from 2012 to 2018. Size distributions of lobsters in the non-MPA sites (Mohawk reef, Carpinteria, and Arroyo Quemado) appear relatively similar in both 2012 and 2018.
